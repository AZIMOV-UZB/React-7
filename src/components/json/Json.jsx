...
const Json = () => {
  ...
  const skeletonItems = new Array(8).fill().map(( idx) => (
  ...
  const productItem = products?.map((product) => (
    ...
    >
     ...
     <button onClick={()=>navigate("/shopping")} className="btr w-9 h-9 rounded-full border-none bg-yellow-400 ">
  ...
  return (
    <div className="container mx-auto mb-16 px-4 py-5">
      ...
      >
        <li  
          data-value={``}
          className="collection border py-1 px-2 rounded-full bg-amber-300	 mb-5  dark:text-white dark:bg-lime-700"
          onClick={(e) => setSelectCategory (e.target.dataset.value)}
        >
          All
        </li>
